...
#include "hoomd/md/ComputeThermoTypes.h"
...
#ifndef AZPLUGINS_COMPUTE_THERMO_SLLOD_H_
#define AZPLUGINS_COMPUTE_THERMO_SLLOD_H_
...
class PYBIND11_EXPORT ComputeThermoSLLOD : public Compute
    ...
    Scalar getTemperature()
        ...
        if (m_group->getTranslationalDOF() + m_group->getRotationalDOF() > 0)
            ...
            return prefactor
                   * (h_properties.data[md::thermo_index::translational_kinetic_energy]
                      + h_properties.data[md::thermo_index::rotational_kinetic_energy]);
            }
        else
            {
            return 0.0;
...
// return the pressure
            ...
            ArrayHandle<Scalar> h_properties(m_properties,
                                             access_location::host,
                                             access_mode::read);
            return h_properties.data[md::thermo_index::pressure];
            }
        else
            {
    ...
    Scalar getTranslationalKineticEnergy()
        ...
        ArrayHandle<Scalar> h_properties(m_properties, access_location::host, access_mode::read);
        return h_properties.data[md::thermo_index::translational_kinetic_energy];
        }
    ...
    Scalar getRotationalKineticEnergy()
        ...
        if (m_computed_flags[pdata_flag::rotational_kinetic_energy])
            ...
            ArrayHandle<Scalar> h_properties(m_properties,
                                             access_location::host,
                                             access_mode::read);
            return h_properties.data[md::thermo_index::rotational_kinetic_energy];
            }
        else
            {
            return 0.0;
    ...
    Scalar getPotentialEnergy()
        ...
        ArrayHandle<Scalar> h_properties(m_properties, access_location::host, access_mode::read);
        return h_properties.data[md::thermo_index::potential_energy];
        }
    ...
    /*! \returns Instantaneous virial tensor, or virial tensor containing NaN entries if it is
        not available
    */
    md::PressureTensor getPressureTensor()
        {
        ...
        if (m_computed_flags[pdata_flag::pressure_tensor])
            ...
            p.xx = h_properties.data[md::thermo_index::pressure_xx];
            p.xy = h_properties.data[md::thermo_index::pressure_xy];
            p.xz = h_properties.data[md::thermo_index::pressure_xz];
            p.yy = h_properties.data[md::thermo_index::pressure_yy];
            p.yz = h_properties.data[md::thermo_index::pressure_yz];
            p.zz = h_properties.data[md::thermo_index::pressure_zz];
            }
        else
            {
    ...
    pybind11::list getPressureTensorPython()
        ...
        md::PressureTensor p = getPressureTensor();
...
#endif //AZPLUGINS_COMPUTE_THERMO_SLLOD_H__
